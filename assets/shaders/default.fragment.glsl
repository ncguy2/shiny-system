...
uniform bool u_photonEnabled = false;
uniform sampler3D u_photonVolume;
uniform float u_photonIntensity;
uniform vec3 u_photonOrigin;
uniform vec3 u_photonExtents;

vec3 convertToPhotonCoords(vec3 worldCoords) {
    vec3 min = u_photonOrigin - u_photonExtents;
    vec3 max = u_photonOrigin + u_photonExtents;

    vec3 tmpMax = max - min;
    vec3 tmpCoords = worldCoords - min;

    return tmpCoords / tmpMax;
}

vec3 samplePhoton(vec3 worldCoords) {
    vec3 coords = convertToPhotonCoords(worldCoords);
    if(coords.x < 0.0 || coords.x > 1.0
     || coords.y < 0.0 || coords.y > 1.0
     || coords.z < 0.0 || coords.z > 1.0) {
        return vec3(0, 0, 0);
     }
     return texture(u_photonVolume, coords).rgb * u_photonIntensity;
}
...
void main() {
    ...
    if(u_photonEnabled) {
        vec3 photonSample = samplePhoton(WorldPosition);
        gl_FragColor.rgb += photonSample;
    }
